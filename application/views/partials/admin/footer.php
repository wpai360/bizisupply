 <footer class="main-footer">
    ...
    <strong>Copyright &copy; 2018 <a href="https://adminlte.io">HawkiSupply</a>.</strong> All rights
    reserved.
  </footer>
...
tinymce.init({
    selector: '#myTextareacreatetestimonial',
    height: 180,
    theme: 'modern',
    plugins: [
  ...
  $('#uploadFile').change(function(e){
      var fileExtension = ['jpg','jpeg','png','gif'];
        if ($.inArray($(this).val().split('.').pop().toLowerCase(), fileExtension) == -1) {
          $('#error_msg_image').html('invalid file extension!');
        }else{
...
<script type="text/javascript">
  ...
  $('#part_logo').submit(function(e) {
      //e.preventDefault();
      var part_logo = $('#uploadFile').val(); 
      //alert("ok");
      if(part_logo==''){
        $('#error_msg_image').html('Please select Logo');
        return false;
      }else{
        $('#error_msg_image').html('');
      }
...
<script>
  $(function () {
    $("#category_listingTable").DataTable();
  });
</script>
<script type="text/javascript" >
...
function recentNotificationView(){       
      $.ajax({
        url: "<?php echo base_url('admin/new-notifications');?>",
        type: 'POST',
        dataType: 'JSON', 
        success: function(data) {
        //console.log(data);
        jQuery('#msg-notif').html(data[0]);
        jQuery('#total').html('You have <b>'+data[1]+'</b> notifications');
        if(data[2]==1){
          jQuery('#newnoti').text(data[2]);
          jQuery('#newnoti').show();
        }else{
          jQuery('#newnoti').hide();
        }
...
jQuery(document).ready(function(){   
  setInterval(function(){ recentNotificationView() },1000);
    function recentNotificationView(){       
      $.ajax({
        url: "<?php echo base_url('admin/new-notifications');?>",
        type: 'POST',
...
$("#noti-update").click(function(){
    NotificationUpdate();
}); 
  function NotificationUpdate(){       
      $.ajax({
        url: "<?php echo base_url('admin/notifications-update');?>",
        type: 'POST',
        dataType: 'JSON', 
        success: function(data) {
        console.log(data);        
