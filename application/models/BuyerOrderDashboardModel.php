...
class BuyerOrderDashboardModel extends CI_Model
{
    ...
    public function ViewProductQuoteList($order_id)
    {
        ...
        $this->db->join('offer_list', 'supplier_marked_offer.offer_id_fk = offer_list.offer_id');
        $this->db->join('users', 'offer_list.supplier_user_id=users.id');
        $this->db->where(['offer_list.order_random_id' => $order_id, 'supplier_marked_offer.form_status' => 1]);
        $query = $this->db->get();
        return $query->result();
    }
    ...
    public function acceptOffer(
        ...
        $offerSent = [
            'request_wait_response' => 1, 'supplier_reject_buyerOffer_accepted' => 0, 'product1_status' => $p1,
    ...
    public function allOrderHistory($user_id)
    {
        ...
        $this->db->where(['offer_list.buyer_user_id' => $user_id, 'supplier_marked_offer.buyer_payment_mark_paid' => 1, 'supplier_payment_mark_received' => 1, 'buyer_delivery_transit_status' => 1, 'supplier_delivery_transit_status' => 1]);
        $this->db->order_by("offer_list.offer_id", "DESC");
        $query = $this->db->get();
        return $query->result();
    }
